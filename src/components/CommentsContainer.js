...
const Comment = ({ data }) => {
  ...
  return (
    <div className="my-3 flex items-center gap-3 p-1 rounded-lg w-full">
      ...
      <div className="w-3/4">
        <p className="font-semibold text-sm text-yt-white">{authorDisplayName}</p>
        <p className="text-xs font-medium ml-5 text-yt-white">{textDisplay}</p>
      </div>
    </div>
  );
};
    ...
    <div>
      ...
      {comment.replies && (
        <div className="ml-20 my-2 pl-3 border-l border-l-gray-500 ">
          <div
            onClick={handle}
            className="flex items-center gap-2 cursor-pointer font-medium text-blue-600 text-yt-white"
          >
...
const CommentsContainer = ({ videoId }) => {
  ...
  return (
    <div>
      <h1 className="font-bold m-1 ml-3 mt-3 text-2xl tracking-wider text-yt-white">
