...
const SearchVideoCard = ({ result }) => {
  const { snippet } = result;
  const { channelTitle, thumbnails, title } = snippet;
  return (
    <div className="flex w-56 flex-col md:flex-row md:w-full ml-4 mr-4 text-yt-white ">
      <img
        alt="thumbnail"
      ...
      <ul className="flex flex-col justify-between flex-wrap">
        <li className="font-bold py-2 text-yt-white">{title}</li>
